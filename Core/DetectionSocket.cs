...
{
    public sealed class SingletonDetection
    {
        public static Socket socket;
        private static SingletonDetection instance = null;
        ...
        public static SingletonDetection Instance
        {
            get
            {
                lock (padlock)
                {
                    if (instance == null)
                    {
                        instance = new SingletonDetection();                        
                    }
                    return instance;
                }
            }
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(content))
                    {
                        ...
                        _hub.Clients.All.SendCoreAsync("ReceiveMessage", new object[] { "Detection: ", content });
                        state.sb.Clear();
                    }
    ...
    //    private static IWebHostEnvironment _env;
    //    private static Microsoft.AspNetCore.SignalR.IHubContext<Core.ClientHub> _hub;
    //    public static void StartListening(IWebHostEnvironment env, Microsoft.AspNetCore.SignalR.IHubContext<Core.ClientHub> hub, bool mode)
    //    {
    //        // Establish the local endpoint for the socket.  
    //        // The DNS name of the computer  
    //        // running the listener is "host.contoso.com".  
    ...
    //                //string name = "todo-filename-" + DateTime.Now.ToString("dd-MM-yyyy") + ".txt";
    //                //File.AppendAllText(Path.Combine(path, name), content + "\n");
    //                _hub.Clients.All.SendCoreAsync("ReceiveMessage", new object[] { "", content });
    //                state.sb.Clear();
    //            }
    //            if (content.IndexOf("<EOF>") == -1)
    //            {
